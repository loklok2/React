...
export default function Traffic() {
    ...
    useEffect(() => {
        if (!tdata || !c1) return;
        console.log("c1=", c1);
    }, [c1, tdata]); // 변경: c1과 tdata를 의존성 배열에 추가
    ...
    useEffect(() => {
        ...
        setC2(tm);
    }, [selC1, tdata, c1]); // 변경: selC1, tdata, c1을 의존성 배열에 추가
    ...
    useEffect(() => {
        ...
        tm = infoKey.map(item => (
            <div key={item} className="flex flex-col">
                <div className="flex justify-center items-center h-10 bg-lime-900 text-white font-bold">
                    {item}
                </div>
                <div className="flex justify-center items-center h-10 bg-lime-100 bg-lime-900 font-bold">
                    {parseInt(tm[item]).toLocaleString()}
                </div>
            </div>
        ));
        setInfo(tm);
    }, [selC2, tdata, c1, selC1]); // 변경: selC2, tdata, c1, selC1을 의존성 배열에 추가
